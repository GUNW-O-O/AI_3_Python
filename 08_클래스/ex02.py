class Student:
    ...
    # * 미리 정의된 이름이며, 특수한 상황에서도 호출되도록 정의되어있다.
    # __init__      : 생성자
    # __del__       : 소멸자
    # __str__       : 객체를 출력할 때 호출되는 메소드
    # __eq__        : 인스턴스가 같은지 비교하는 메소드
    # __ne__        : 인스턴스가 같은지 비교하는 메소드
    
    def __str__(self):
        return '{} / {} / {}'.format(self.no, self.name, self.major)
    
    # A == B -> __eq__ 호출
    # __eq__ 를 정의하지 않으면, A == B 는 인스턴스를 비교
    def __eq__(self,obj):
        return self.no == obj.no
    
    # A != B -> __ne__ 호출
    # __ne__ 를 정의하지 않으면, A != B 는 인스턴스를 비교
    def __ne__(self, obj):
        return self.no != obj.no
...
students = [
    ...
    Student(102, '박조은', '정치학과'),
    Student(103, '이조은', '법학과'),
    Student(104, '최조은', '컴퓨터과학과')
]
